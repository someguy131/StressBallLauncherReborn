...
#define intakeMotor 5
#define launcherMotor 6
...
void setup() {
    ...
    MCLeft.attach(driveLeft);
    Serial.println("left started");
    MCLeft.writeMicroseconds(1500);
  
    MCRight.attach(driveRight);
    Serial.println("right started");
    MCRight.writeMicroseconds(1500);

    MCStrafe.attach(driveStrafe);
    Serial.println("strafe started");
    MCStrafe.writeMicroseconds(1500);

    MCIntake.attach(intakeMotor);
    Serial.println("intake motor started");
    MCIntake.writeMicroseconds(1500);

    MCLauncher.attach(launcherMotor);
    Serial.println("launcher motor started");
    MCLauncher.writeMicroseconds(1500);
}
...
void loop() {
    ...
    if (newData == true) {
        ...
        if(idVal == 2){
          ...
          MCIntake.writeMicroseconds(pwmVal);
          //Serial.println("intake"); 
          //Serial.println(pwmVal);
        }else if(idVal == 6){
          MCLauncher.writeMicroseconds(pwmVal);
    ...
    if(millis()-lastMilli > 50){
      strafeEncoderCount = strafeEncoder.getEncoderCount(); //value from encoder function
      leftEncoderCount = leftEncoder.getEncoderCount(); 
      rightEncoderCount = rightEncoder.getEncoderCount();

      Serial.print("<o,SE,");
      Serial.print(strafeEncoderCount);
      Serial.print(",>\n");
